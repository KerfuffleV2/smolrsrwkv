...
impl Attention {
    pub fn time_mixing_ops(&self, ctx: &Context, state: &mut RWKVLayerState, x: Tensor) -> Tensor {
        ...
        state.tm_last_x = ctx.op_dup(&x);
        state.tm_aa = ctx.op_dup(&new_aa);
        state.tm_bb = ctx.op_dup(&new_bb);
        state.tm_pp = ctx.op_dup(&new_pp);
